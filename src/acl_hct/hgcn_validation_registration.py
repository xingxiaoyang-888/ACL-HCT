...
NEW_SOURCES = ('development_view hgcn_geometry hgcn_panels hgcn_evidence hgcn_statistics '
               ...
               'hgcn_validation').split()
PHASES = ('cpu_fixture', 'cuda_fixture', 'train', 'eval', 'official', 'analyze')
...
def role(config, phase, seed=None, repeat_start=None):
    ...
    if phase in ('train', 'eval'):
        ...
        raise ValueError('seed selector only for WordNet train/eval')
    if phase == 'eval':
...
def verify_release(config, phase, source_commit, record_path, seed=None, repeat_start=None):
    ...
    if phase in ('train', 'eval', 'official', 'analyze'):
    ...
    required = {'eval': ('training_run_sha256', 'best_checkpoint_sha256'),
    ...
    if (type(spent) not in (int, float) or type(reserved) not in (int, float)
            ...
            or spent < config['resources']['spent_quality_GPU_seconds'] or reserved < job['allocation_gpu_seconds']
