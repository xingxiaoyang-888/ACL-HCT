...
所有方法复用同一索引流，记录SHA256；小组合数阈值预先固定2048，超过后固定MC次数，按chunk流式累积。正式配置MC4096、chunk256，N16/d2/c1下固定k=1/2/3/4/8/16、尺度0.05/0.15/0.7与三种构型。不是曲率/维度等全部参数的笛卡尔积。配置上限128个case、N128、单MC100000、总邻域2M；执行工具默认CPU，设备切换需实验任务按既定预算调度。
