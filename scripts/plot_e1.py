...
def plot(result_path, output_dir):
    ...
    modes={r['result']['mode'] for r in rows}
    exact=modes=={'exact'}
    mode_title='Exact enumeration' if exact else ('Monte Carlo' if modes=={'mc'} else 'Mixed exact / MC')
    ...
    axes[0].set_title('Exact expectation bias norm' if exact else 'MC mean norm is noisy, not exact bias')
    ...
    ax.set(xlabel='Tangent construction scale',ylabel='Norm of mean offset minus second-order oracle',
           title='Exact prediction discrepancy' if exact else 'Discrepancy includes MC noise; see JSON covariance')
